...
const GeminiSlingshot: React.FC = () => {
  ...
  const isPathClear = (target: Bubble) => {
    if (!anchorPos.current || !gameContainerRef.current) return false;
    
    const startX = anchorPos.current.x;
    const startY = anchorPos.current.y;
    const canvasWidth = gameContainerRef.current.clientWidth || window.innerWidth;
    
    const checkVirtualPath = (vX: number, vY: number) => {
        const dx = vX - startX;
        const dy = vY - startY;
        ...
        for (let i = 1; i < steps - 2; i++) { 
            const t = i / steps;
            const virtualPx = startX + dx * t;
            const py = startY + dy * t;
            
            let cx = virtualPx;
            const boundaryL = BUBBLE_RADIUS;
            const boundaryR = canvasWidth - BUBBLE_RADIUS;
            
            if (cx < boundaryL) {
                cx = boundaryL + Math.abs(boundaryL - cx);
            } else if (cx > boundaryR) {
                cx = boundaryR - Math.abs(cx - boundaryR);
            }
            
            for (const b of bubbles.current) {
                if (!b.active || b.id === target.id) continue;
                const distSq = Math.pow(cx - b.x, 2) + Math.pow(py - b.y, 2);
    ...
    if (checkVirtualPath(target.x, target.y)) return true;
    
    const leftWall = BUBBLE_RADIUS;
    const leftVirtualX = leftWall - (target.x - leftWall);
    if (checkVirtualPath(leftVirtualX, target.y)) return true;
    
    const rightWall = canvasWidth - BUBBLE_RADIUS;
    const rightVirtualX = rightWall + (rightWall - target.x);
    if (checkVirtualPath(rightVirtualX, target.y)) return true;
